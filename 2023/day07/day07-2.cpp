...
const char cards[] = {'d', 'c', 'b', 'a', '9', '8', '7', '6', '5', '4', '3', '2', '1'};
...
HandType getHandType(const Hand& hand) {
    ...
    int highestIndex = 0;
    for (int i = 1; i < NUM_CARD_VALUES - 1; i++) {
    ...
    highestCount += count[NUM_CARD_VALUES - 1];
...
int main(int argc, char* argv[]) {
    ...
    if (file.is_open()) {
        while (getline(file, line)) {
            ...
            std::replace(hand.cards.begin(), hand.cards.end(), 'A', 'd');
            std::replace(hand.cards.begin(), hand.cards.end(), 'K', 'c');
            std::replace(hand.cards.begin(), hand.cards.end(), 'Q', 'b');
            std::replace(hand.cards.begin(), hand.cards.end(), 'T', 'a');
            std::replace(hand.cards.begin(), hand.cards.end(), 'J', '1');
