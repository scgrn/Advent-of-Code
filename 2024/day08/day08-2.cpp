...
bool inBounds(const Coord& antinode) {
    ...
    for (auto antenna : antennas) {
        ...
        for (int i = 0; i < antennaList.size(); i++) {
            for (int j = 0; j < antennaList.size(); j++) {
                if (i != j) {
                    int dx = antennaList[j].x - antennaList[i].x;
                    int dy = antennaList[j].y - antennaList[i].y;

                    Coord antinode;
                    antinode.x = antennaList[j].x;
                    antinode.y = antennaList[j].y;
